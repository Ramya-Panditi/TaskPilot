...
function Lists() {
  ...
  const handleCheckboxChange = (id,status) => {
    ...
    try {
      axios
      .put(`${baseURL}/updatestatus/${sessionStorage.getItem("id")}/${id}`)
      .then((res) => {
        if (res.status === 200) {
  ...
  const confirmDeleteTask = () => {
    if (taskid !== null) {
      try {
        axios.delete(`${baseURL}/delete-task/` + sessionStorage.getItem("id")+ "/" + taskid + "/")
        .then((res) => {
          if (res.status === 200) {
    ...
    if (editedTask) {
      try {
        ...
        const response = await axios.put(
          `${baseURL}/updatetask/${sessionStorage.getItem("id")}/${editedTask._id}`,
          updatedTask
          );
